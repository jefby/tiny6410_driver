/*
	驱动程序:Tiny 6410 MLC2驱动程序(K9GAG08U0E)
	
	未实现ECC功能,因为S3C6410 NAND FLASH控制器最多支持到8-bit的ECC，而NAND FLASH芯片却需要24-bitECC，所以不太好做，友善之臂又不开源.
	另外软件ECC功能经实践不可用.
	将源码放入drivers/mtd/nand/中,并在Makefile中增加一行obj-y += 该源文件名称.o
	重新编译内核
...
static struct mtd_partition tiny6410_nand_mlc2[] = {
	...
	{
		.name		= "File System",
		.offset		= MTDPART_OFS_APPEND,//剩下的都可以存放
...
static int samsung_nand_init(void)
{
	...
	//将NAND FLASH控制寄存器映射到虚拟地址空间s3c_nand_reg_ptr
...
#define TWRPH1 0
	...
	s3c_nandchip->ecc.mode = NAND_ECC_NONE;//!!很关键,不使用ECC校验
	//4.使用nand_scan函数
	...
	nand_scan(s3c_mtdinfo,1);//检查是否能发现NAND FLASH芯片
	//5.add_mtd_partions
	add_mtd_partitions(s3c_mtdinfo,tiny6410_nand_mlc2,3);//将分区表添加到内核中去
...
{
	del_mtd_partitions(s3c_mtdinfo);//删除分区信息
