...
static void do_ramblk_request(struct request_queue *q )
{
	...
	while (req) {
		...
		unsigned offset = blk_rq_pos(req)<<9;//source / dest
		unsigned len = blk_rq_cur_sectors(req)<<9;//长度

		if (req->cmd_type != REQ_TYPE_FS)
			goto done;
	
		if(rq_data_dir(req) == READ){
			memcpy(req->buffer,ramblk_buf+offset,len);//拷贝到缓冲区
		}else {
			memcpy(ramblk_buf+offset,req->buffer,len);//写缓冲区到磁盘
		}
		done:
...
//	3.硬件相关的操作
	ramblk_buf = (char*)kzalloc(RAMBLK_SIZE,GFP_KERNEL);//申请RAMBLK_SIZE内存
