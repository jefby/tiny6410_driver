...
static struct button_irq_desc * buttons_devid = NULL;//在定时器到时函数中使用

void buttons_timer_function(unsigned long data)
{
	unsigned char cmp = 0;
	volatile unsigned long rval = 0;
	struct button_irq_desc *temp = buttons_devid ;
	if(!temp)
		return;
	cmp = temp->number;
	if(cmp >=0 && cmp < 6 )
...
//中断处理程序，记录下发生中断的ID号，保存到全局变量buttons_devid上，然后修改定时器使之延长10ms，用于消除抖动
static irqreturn_t buttons_interrupt(int irq,void *dev_id)
{	
	buttons_devid = (struct button_irq_desc *)dev_id;

	mod_timer(&buttons_timer,jiffies + HZ/100);//修改定时器的超时时间,10ms后启动定时器
...
{ 
   ...
   //申请中断号
	...
	if (button_irqs[i].irq < 0) {
        ...
        err = request_irq(button_irqs[i].irq, buttons_interrupt, IRQ_TYPE_EDGE_BOTH, //双边沿触发
...
{
	del_timer(&buttons_timer);//delete the timer 
