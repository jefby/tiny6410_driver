...
#include <linux/module.h>//MODULE_LICENST,MODULE_AUTHOR
...
#define BUF_SIZE (1024)//定义拷贝数据字节数512KB
...
long dma6410_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case TRANS_USE_DMA://启动方式:手工启动
			...
			//8.写控制信息
			dma_chan_p->DMACCxControl0 = (1<<31) | (1 << 27) | (1 <<26)  | (0 <<25) | (0<<24) | (2<<21) | (2<<18)  | (0<<15)  | (0<<12); /*Destination increment*/
						//(1<<31) Terminal count interrupt enable bit.
			...
			//9.写通道配置信息，启动通道0
			dma_chan_p->DMACCxConfiguration = (0 << 18) | (0 << 11)| (1 << 0) | (1 << 15);
											
			//(0 << 18) allow DMA request
			//(0 << 11) Flow control and transfer type ; Memory to Memory
			//(1 << 0) channel enabled
			//(1<<15) Terminal count interrupt mask. When cleared this bit masks out the terminal count interrupt of the relevant channel.

			printk("configure ok.\n");
			//如果DMA传输未完则睡眠
			wait_event_interruptible(sdma_waitq, ev_dma);
			printk("TRANS_USE_DMA ok.");
			//检查拷贝是否正确
			if(memcmp(src,dst,BUF_SIZE) == 0)
			{
				printk("TRANS_USE_DMA ok.");
			}else{
				printk("TRANS_USE_DMA err.\n");
			}
...
static int dma6410_init(void)
{
	int ret = 0;
	//中断请求
	ret = request_irq(IRQ_SDMA0, sdma_irq,0,"sdma-0",0);
	if(ret){
	...
	if(!dmac_p->DMACEnbldChns&0x1){
		printk("chan 0 can use.\n");
		dmac_p->DMACEnbldChns |= 0x1;//启用通道0
		
		dma_chan_p = (struct s3c6410_dma_chan *)((volatile unsigned long*)dmac_p+0x100/4);
	}
	//2.决定使用SDMAC还是通用的DMAC
	sdma = ioremap(0x7E00F110,16);//SDMA_SEL ,设置使用SDMA
	iowrite32(0,sdma);
	iounmap(sdma);//清除映射
